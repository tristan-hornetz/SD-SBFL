...
def get_test_ids():
    ...
    with open(info_dir + "/run_test.sh", "rt") as f:
        while True:
            ...
            if (
                run_test_sh.startswith("python")
                or run_test_sh.startswith("pytest ")
                or run_test_sh.startswith("tox")
                or run_test_sh.startswith("py.test")
            ):
                ret.append(
                    list(
                        filter(lambda s: not s.startswith("-"), run_test_sh.split(" "))
                    )
                    .pop()
                    .split("[")[0]
                )
...
def get_subprocesses(pid: int):
    sp = subprocess.Popen(
        ["ps", "-opid", "--no-headers", "--ppid", str(pid)],
        encoding="utf8",
...
def run_test(
    root_dir: str, project: str, bug_id: int, output_file=dump_file, work_dir=""
):
    """
    ...
    os.system(
        f"{binary_dir}/bugsinpy-checkout -p {project} -i {bug_id} -v 0 -w {os.path.dirname(work_dir)}"
    )
    os.system(f"{binary_dir}/bugsinpy-compile -w {work_dir}")
    if os.path.exists(f"{work_dir}/bugsinpy_setup.sh"):
        print("\n=========setup============\n")
        os.system(
            f"cp {binary_dir}/alltest_template.sh {work_dir}/bugsinpy_modified_setup.sh"
        )
        os.system(
            f"cat {work_dir}/bugsinpy_setup.sh >> {work_dir}/bugsinpy_modified_setup.sh"
        )
...
if __name__ == "__main__":
    ...
    arg_parser = argparse.ArgumentParser(
        description="Run the debugger on a specific bug"
    )
