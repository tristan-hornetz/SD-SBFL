...
class StatisticsExtractor:
    def __init__(self, methods, all_types: List[type]):
        ...
        self.absolute_type_counts = {
            t: sum(
                info.type_counts[t] if t in info.type_counts.keys() else 0
                for info in infos
            )
            for t in all_types
        }
        self.average_type_counts = {
            t: self.absolute_type_counts[t] / self.method_count for t in all_types
        }
        self.average_subnode_count = (
            sum(info.subnode_count for info in infos) / self.method_count
        )
        self.average_expression_count = (
            sum(info.num_expressions for info in infos) / self.method_count
        )
        self.average_statement_count = (
            sum(info.subnode_count - info.num_expressions for info in infos)
            / self.method_count
        )
        self.unique_names_per_method = (
            sum(len(info.variable_names.keys()) for info in infos) / self.method_count
        )
        self.name_references_per_method = (
            sum(sum(info.variable_names.values()) for info in infos) / self.method_count
        )
...
class CodeStatistics:
    def __init__(self, all_methods, buggy_methods):
        all_types = list(
            set.union(*(set(m.method_info.type_counts.keys()) for m in all_methods))
        )
